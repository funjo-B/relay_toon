...
"feat-solo-3": () => S(win(140,60,1320,780) + `
  ${txt(800,170,"『병맛 고등학교』 공개 설정",32,C.txt,800,"middle")}
  ${[["작품 공개","모두가 감상할 수 있어요",true,250],
     ["브랜치(평행세계) 허용","다른 창작자가 내 작품에서 분기 가능",true,370],
     ["팬 참여 이벤트","특정 화를 릴레이로 개방",false,490]].map(([t,d,on,y])=>
    card(360,y,880,100,C.card,C.line,18)+txt(400,y+45,t,21,C.txt,700)+txt(400,y+75,d,15,C.sub)+toggle(1140,y+35,on)).join("")}
  ${card(360,610,880,90,C.card2,C.acc,18)}
  ${aiTag(390,632)}
  ${txt(460,665,"브랜치 허용 시 원작 Story Bible이 분기 시점 기준으로 자동 복사됩니다.",16,C.sub)}
  ${uiBtn(640,730,320,64,"저장하기","primary",21)}
`),
...
"feat-collab-3": () => S(win(140,60,1320,780) + `
  ...
  ${txt(220,590,"⚠ 2컷의 철수에게 안경이 없습니다 — Story Bible 기준 철수는 안경 캐릭터예요.",19,"#ffb3ad",700)}
...
"feat-open-1": () => S(win(140,60,1320,780) + `
  ...
  ${txt(220,637,"📋 릴레이 규칙 : 턴당 1~3컷 · 병맛 유지 · 기존 캐릭터 설정 준수 (AI가 검증)",16,C.sub)}
...
"feat-branch-2": () => S(win(140,60,1320,780) + `
  ...
  ${[["👤 인물 12","철수(안경) · 영희 · 그림자 인물 外"],
     ...
     ["🧩 떡밥 5","지하실의 비밀 · 그림자 정체 外"]].map(([k,v],i)=>
